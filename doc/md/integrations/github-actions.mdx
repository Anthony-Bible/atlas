...
jobs:
  push:
    services:
      # Spin up a Clickhouse container to be used as the dev-database.
      clickhouse:
...
can use the [`atlas-action/migrate/lint`](https://github.com/anthony-bible/atlas-action/blob/master/migrate/lint/action.yml) GitHub Action.
...
jobs:
  lint:
    services:
      # Spin up a Clickhouse container to be used as the dev-database for analysis.
      clickhouse:
        image: clickhouse/clickhouse-server:23.10
        env:
          CLICKHOUSE_DB: test
...
You can use [ariga/atlas-action/migrate-apply](https://github.com/anthony-bible/atlas-action/blob/master/migrate/apply/action.yml) to deploy migrations to your database directly from
GitHub Actions.
